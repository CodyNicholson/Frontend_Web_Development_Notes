...
function numberAdder ()
{
    ...
    text = $(this).text();
    // 'this' refers to the current 'p' tag that is selected, which will have its text stored in the text var

    number = text.length;
    // We find the length of the text var that we saved above and store it in the number var

    $(this).text(text + " " + number);
    // Last, we set the value of the current 'p' tags text to its original text plus a space and then the number of characters in between the 'p' tag without the space and number at the end
}

$('p').each(numberAdder);
// The each() method will go through every 'p' tag in the html file and execute the numberAdder method on that 'p' tag
```
